...
| # | 命题 | 证据 |
|---|---|---|
| A1 | 两节点运行后，各自 `cluster_nodes` 都能看到对方且标记 online | 待实测（双实例联调） |
| A2 | A `cluster_send` → B 会话收到注入消息，前缀含 `[cluster:A]` | 待实测 |
| A3 | A `cluster_broadcast` → 其余节点都收到，A 自己**不**收到 | 待实测 |
| A4 | 同一 messageId 重复投递只注入一次 | 待单测 |
| A5 | 目标 agent 不可用时消息保留原位，agent 就绪后仍投递 | 待单测 + 实测 |
| A6 | 心跳超过 `offlineAfterMs` → 名册标 offline；在线计数同步变化 | 待单测 |
| A7 | 坏 JSON / 缺字段 / 超长 text / 未来版本 → 跳过并落痕，插件不崩 | 待单测（尸体测试） |
| A8 | 在线判定、名册显示、发送警告共用同一阈值常量（判据单源） | 待单测（断言同一常量） |
| A9 | dispose 后无残留定时器、自身心跳文件已移除、无文件句柄泄漏 | 待单测（HMR 安全） |
| A10 | 写失败（只读目录）时工具返回明确错误，不静默；轨迹仍可写时不崩 | 待单测（尸体测试） |
| A11 | 注入消息可从会话事件流重建（Model-visible ⟺ logged） | 待实测（会话日志核对） |
